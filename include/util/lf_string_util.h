#ifndef LF_STRING_UTIL_H
#define LF_STRING_UTIL_H
#include <cstring>
#include <algorithm>
#include "util/lf_ascii.h"
namespace LF
{
	namespace lf_string_util
	{
		template <class S>
			...
			/// whitespace removed.
		{
			...
			while (it != end && lf_ascii::isSpace(*it)) ++it;
			return S(it, end);
		}
			...
			/// whitespace removed.
		{
			...
			while (pos >= 0 && lf_ascii::isSpace(str[pos])) --pos;
			...
			/// trailing whitespace removed.
		{
			...
			while (first <= last && lf_ascii::isSpace(str[first])) ++first;
			while (last >= first && lf_ascii::isSpace(str[last])) --last;
			...
			/// Returns a copy of str containing all upper-case characters.
		{
			...
			while (it != end) result += static_cast<typename S::value_type>(lf_ascii::toUpper(*it++));
			return result;
		}
			...
			/// Replaces all characters in str with their upper-case counterparts.
		{
			...
			while (it != end) { *it = static_cast<typename S::value_type>(lf_ascii::toUpper(*it)); ++it; }
			return str;
		}
			...
			/// Returns a copy of str containing all lower-case characters.
		{
			...
			S result;
			result.reserve(str.size());
			while (it != end) result += static_cast<typename S::value_type>(lf_ascii::toLower(*it++));
			return result;
		}
			...
			/// Replaces all characters in str with their lower-case counterparts.
		{
			...
			while (it != end) { *it = static_cast<typename S::value_type>(lf_ascii::toLower(*it)); ++it; }
			return str;
		}
			...
			/// Case-insensitive string comparison
		{
			...
			{
				typename S::value_type c1(static_cast<typename S::value_type>(lf_ascii::toLower(*it1)));
				typename S::value_type c2(static_cast<typename S::value_type>(lf_ascii::toLower(*it2)));
				if (c1 < c2)
					return -1;
				else if (c1 > c2)
					return 1;
			...
			const typename S::value_type* ptr)
		{
			...
			while (it != end && *ptr)
			{
				typename S::value_type c1(static_cast<typename S::value_type>(lf_ascii::toLower(*it)));
				typename S::value_type c2(static_cast<typename S::value_type>(lf_ascii::toLower(*ptr)));
				if (c1 < c2)
					return -1;
				else if (c1 > c2)
					return 1;
		...
		struct i_char_traits : public std::char_traits<charT>
		{
			...
			{
				return lf_ascii::toLower(c1) == lf_ascii::toLower(c2);
			}
			...
			static int compare(const charT* s1, const charT* s2, size_t n)
			{
				...
				{
					if (lf_ascii::toLower(*s1) == lf_ascii::toLower(*s2)) continue;
					else if (lf_ascii::toLower(*s1) < lf_ascii::toLower(*s2)) return -1;
					else return 1;
				}
			...
			{
				while (n-- > 0 && lf_ascii::toLower(*s) != lf_ascii::toLower(a)) { ++s; }
				return s;
			}
		};
